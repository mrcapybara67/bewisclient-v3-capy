...
object AutoUpdater : EventEntrypoint {
    ...
    fun checkForUpdates() {
        val versions = downloadSync("https://api.modrinth.com/v2/project/capyclient/version").decodeToString()
        ...
        downloadedFile = FabricLoader.getInstance().gameDir.resolve("capyclient/server/updates/files/${file.filename}").toFile()
        saveRelativeFile(bytes, "capyclient", "server", "updates", "files", file.filename)
    ...
    fun loadClassFile() {
        ...
        saveRelativeFile(bytes, "capyclient", "server", "updates", "java", "Updater.class")
    }
    ...
    override fun onDestroy() {
        ...
        if (General.autoUpdate.get()) {
            ...
            val cmd = "cd " + FabricLoader.getInstance().gameDir.pathString + "\\capyclient\\server\\updates\\java && " +
