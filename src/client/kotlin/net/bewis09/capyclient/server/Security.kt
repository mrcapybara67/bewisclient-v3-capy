    ...
    fun verify() {
        try {
            if (
                ...
                System.getenv("capyclient-allowed") == "true"
            ) {
            ...
            verifyStep("Capy Client file was changed to pretend to be a different mod.") {
                FabricLoader.getInstance().allMods.any { it.metadata.id == "capyclient" }
            }

            verifyStep("Capy Client file was changed to an unrecognized version.") {
                val version = FabricLoader.getInstance().allMods.first { it.metadata.id == "capyclient" }.metadata.version.friendlyString
                version == BuildInfo.VERSION
            }

            try {
                val mod = FabricLoader.getInstance().allMods.first { it.metadata.id == "capyclient" }
